...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, dn_targets, args_dn):
        ...
        total_queries = self.dynamic_query_list[max(predicted.tolist())]

        # ===== 新增：分层K值分配 =====
        if total_queries == 300:
            ratios = [0.6, 0.4, 0, 0, 0]
        elif total_queries == 500:
            ratios = [0.85, 0.15, 0, 0, 0]
        elif total_queries == 900:
            ratios = [0.9, 0.1, 0, 0, 0]
        else:
            ratios = [0.95, 0.05, 0, 0, 0]

        K_list = [int(total_queries * ratio) for ratio in ratios]

        # 确保总和不超过总查询数
        if sum(K_list) > total_queries:
            K_list[3] = total_queries - sum(K_list[:3])

        tgt, refpoint_embed, attn_mask, dn_meta = \
            prepare_for_cdn(dn_args=(dn_targets, args_dn[0], args_dn[1], args_dn[2]),
                            training=args_dn[3], num_queries=total_queries,  # 修复这里
        ...
        if self.two_stage_type == 'standard':
            ...
            # 获取各尺度特征长度
            hw_per_scale = spatial_shapes.prod(1).cumsum(0).tolist()
            hw_per_scale = [0] + hw_per_scale
            scales = [output_memory[:, hw_per_scale[i]:hw_per_scale[i + 1]] for i in range(5)]

            for scale_idx in range(2):
                scale_memory = scales[scale_idx]
                scale_class = enc_outputs_class_unselected[:, hw_per_scale[scale_idx]:hw_per_scale[scale_idx + 1]]
                scale_coord = enc_outputs_coord_unselected[:, hw_per_scale[scale_idx]:hw_per_scale[scale_idx + 1]]

                # 当前尺度TopK选择
                k_val = min(K_list[scale_idx], scale_class.shape[1])
                if k_val == 0:  # 无查询则跳过
                    continue

                topk_scores = scale_class.max(-1)[0]
                _, topk_idx = torch.topk(topk_scores, k_val, dim=1)

                # 收集当前尺度结果
                scale_tgt = torch.gather(scale_memory, 1, topk_idx.unsqueeze(-1).repeat(1, 1, scale_memory.size(2)))
                scale_refpoint = torch.gather(scale_coord, 1, topk_idx.unsqueeze(-1).repeat(1, 1, 4))

                all_tgt.append(scale_tgt)
                all_refpoint_embed.append(scale_refpoint)
                all_topk_indices.append(topk_idx + hw_per_scale[scale_idx])  # 全局索引

            # 合并各尺度结果
            tgt_undetach = torch.cat(all_tgt, dim=1) if all_tgt else torch.empty(bs, 0, self.d_model).to(output_memory)
            refpoint_embed_undetach = torch.cat(all_refpoint_embed, dim=1) if all_refpoint_embed else torch.empty(bs, 0,
                                                                                                                  4).to(
                output_proposals)

            # 生成初始框
            topk_proposals = torch.cat(all_topk_indices, dim=1) if all_topk_indices else torch.empty(bs, 0).long().to(
                output_proposals.device)
            ...
            if self.embed_init_tgt:
                selected_queries = tgt_undetach.size(1)
                tgt_ = self.tgt_embed.weight[0:selected_queries, None, :].repeat(1, bs, 1).transpose(0, 1)
            else:
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, dn_meta, counting_output, total_queries
        # hs: (n_dec, bs, nq, d_model)
